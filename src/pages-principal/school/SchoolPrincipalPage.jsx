...
const SchoolPrincipalPage = () => {
  ...
  useEffect(() => {
    const fetchSchoolData = async () => {
      try {
        setLoading(true);
        // 1. Get Principal's Info to find the School ID
        const principalRes = await request('/principals/me', 'GET');
        
        if (principalRes.data && principalRes.data.school_id) {
          // 2. Get School Details
          const schoolRes = await request(`/schools/${principalRes.data.school_id}`, 'GET');
          if (schoolRes.data) {
            setSchool(schoolRes.data);
          }
        } else {
            // Handle case where principal isn't assigned to a school yet
      ...
      <div className="mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-6 mb-6">
          <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
            ...
            <button
              onClick={() => navigate(`/principal/school/edit/${school.id}`)} 
              className="flex items-center gap-2 bg-indigo-600 hover:bg-indigo-700 text-white px-5 py-2.5 rounded-xl font-semibold transition-all shadow-sm hover:shadow-md"
            >
              <Edit size={18} />
